...
class PositionMonitor:
    ...
    def _handle_partial_close(
        ...
        try:
            fresh_exchange = self._fetch_single_exchange_position(symbol)
            if not fresh_exchange:
                close_price = float(exchange_pos.get('mark_price') or exchange_pos.get('entry_price') or ledger_pos.entry_price or 0.0)
                forced = self.ledger.force_close_position(
                    symbol=symbol,
                    realized_pnl=0.0,
                ...
                if forced:
                    ...
                    logger.warning(
                        "PARTIAL_CLOSE_RECONCILE_FLAT_NOOP symbol={} reason=already_flat",
                        symbol,
                    )
                self._partial_close_cooldown_until[symbol] = now + self.partial_close_reconcile_cooldown_seconds
                return

            fresh_size = float(fresh_exchange.get('size', 0.0) or 0.0)
            if fresh_size <= self.partial_close_size_epsilon:
                close_price = float(fresh_exchange.get('mark_price') or fresh_exchange.get('entry_price') or ledger_pos.entry_price or 0.0)
            ...
            diff = abs(float(ledger_pos.size or 0.0) - fresh_size)
            ...
            adjusted = self.ledger.adjust_position_size(
                symbol=symbol,
                new_size=fresh_size,
                avg_price=fresh_exchange.get('entry_price'),
